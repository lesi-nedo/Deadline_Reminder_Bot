...
COURSE_NAME=$(extract_env_suffix "$ENV_FILE")
TEMP_CRON=$(mktemp)
...
echo "# Telegram Bot Reminder ${COURSE_NAME} -- runs dayly at 9:00 AM" >>"$TEMP_CRON"
echo "0 9 * * * $VENV_PATH/bin/python $SCRIPT_FILE $ENV_FILE cron reminder >> $LOG_PATH" >>"$TEMP_CRON"
echo "# Telegram Bot Stats ${COURSE_NAME} -- runs dayly at 9:00 PM" >>"$TEMP_CRON"
